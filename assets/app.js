...
var firebaseConfig = {
  apiKey: "AIzaSyAA3W5qK75FPWcT6xt-KqjGPBWIXDLUH7o",
  authDomain: "projectonetest-c7799.firebaseapp.com",
  databaseURL: "https://projectonetest-c7799.firebaseio.com",
  projectId: "projectonetest-c7799",
  storageBucket: "projectonetest-c7799.appspot.com",
...
$('#append-gif').on('click', function(evt) {
    ...
    if (otherUser !== undefined) {
        console.log("chat with " + otherUser + " ready!");
        firebase.database().ref(otherUser + "-" + realname).push(htmlText);
    }                          
...
$(document).on("click", ".click-user", function(event) {
    event.preventDefault();
    otherUser = $(this).attr("data-other-user");
    currentChat = realname + "-" + otherUser;


    $("#msg-content").hide();
    $("#chat-page").show();
    $("#chat-page").html("<h1 style='color: white;'>This is the individual chat page</h1>");

    firebase.database().ref(currentChat).on("child_added", function(snapshot) {

        $("#chat-page").append(snapshot.val());
        console.log("this goes to chat page, not public page");
        chatAutoScroll();
